...
export function ContentSection({
  ...
  const renderContent = () => {
    if (children) {
      // Affichage custom, garde le flex mais sans scroll horizontal
      return (
    ...
    if (loading) {
      ...
      return (
        <div className="
          ...
          grid-rows-1 sm:grid-rows-2 gap-3
        ">
    ...
    // Responsive grid : 1 ligne x 3 (mobile), 2 lignes x N (desktop)
    return (
      <div
        className={`
          grid
          grid-cols-3 sm:grid-cols-4 md:grid-cols-5 xl:grid-cols-6
          ${isMobile() ? "grid-rows-1" : "grid-rows-2"}
          gap-3
        `}
      >
        {items.slice(0, isMobile() ? 3 : 10).map((item) => (
          <Link
            key={item.id}
          ...
          >
            ...
            <div className="p-1">
              <h3 className={`truncate font-medium text-xs`}>{item.title}</h3>
  ...
  // Utilitaire pour détecter le mobile (window.innerWidth)
  function isMobile() {
